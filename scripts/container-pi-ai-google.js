...
function logToFile(message) {
    try {
        console.log("[GEMINI_DEBUG] " + message);
        fs.appendFileSync('/tmp/gemini_debug.log', message + '\n');
    } catch (e) {
...
export const streamGoogle = (model, context, options) => {
    ...
    (async () => {
        ...
        // PATCH: Strip 'google/' prefix to prevent SDK 404
        if (model.id.startsWith("google/")) {
            logToFile(`Patching model ID: ${model.id} -> ${model.id.replace("google/", "")}`);
            model.id = model.id.replace("google/", "");
        }
        ...
        try {
            ...
            logToFile("--- GOOGLE STREAM STARTED ---");
            ...
            for await (const chunk of googleStream) {
                logToFile("--- CHUNK RECEIVED ---");
                logToFile(JSON.stringify(chunk, null, 2));
            ...
            stream.end();
            logToFile("--- GOOGLE STREAM DONE ---");
        }
        catch (error) {
            logToFile("--- GOOGLE STREAM ERROR ---");
            logToFile(error instanceof Error ? error.message : JSON.stringify(error));
...
export const streamSimpleGoogle = (model, context, options) => {
    ...
    return streamGoogle(model, context, base);
};
